package org.golde.btpr180.image;
...
import org.golde.btpr180.image.filter.FilterDithered;
import org.golde.btpr180.image.filter.IImageFilter;

public class ThermalImage {

	protected final BufferedImage image;

	protected final IImageFilter bitonalAlgorithm;
	...
	public ThermalImage(BufferedImage image) {
		this(image, new FilterDithered());
	}
	
	public ThermalImage(BufferedImage image, IImageFilter bitonalAlgorithm){
		this.image = image;
	...
	protected  List<ByteArrayOutputStream> image2Rows(int bitsPerColumn_8_or_24){
		List<ByteArrayOutputStream> lRasterRows = new ArrayList<ByteArrayOutputStream>();


		List<BufferedImage> lRGBImageRows = new ArrayList<BufferedImage>();
		for(int y = 0; y < image.getHeight();y+=bitsPerColumn_8_or_24){
			int height = bitsPerColumn_8_or_24;
			if((y + height) > image.getHeight()){
				height = image.getHeight() - y;
			}
			BufferedImage row = (BufferedImage) image.getSubimage(0, y, image.getWidth(), height);
